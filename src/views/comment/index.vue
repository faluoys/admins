<template>
    <div>
        ...
        <div class="content px-4">
            ...
            <EditComment v-if="showEditModal" :comment_id="comment_id" :showModal="showEditModal"
                         @checkShowModal="checkEditModal" @reloadTable="reload"></EditComment>
            <DeleteComment v-if="showDelModal" :comment_id="comment_id" :showModal="showDelModal"
                           @checkShowModal="checkDelModal" @reloadTable="reload"></DeleteComment>
        </div>
    </div>
</template>
...
// 筛选已下单商品
const goodSubmit = (rates: number) => {
    formSearch.value.rate = rates;
    repetition();
};

const getCommentList = (params: { include: string; current?: any; goods_title?: string; rate?: number; reply?: string; }) => {
    loadingBar.start();
