<template>
    <div>
        ...
        <div class="content px-4">
            <div class="bg-white pt-4 pr-6">
                ...
                >
                    ...
                    <n-form-item label="是否上架">
                        <n-button type="info" v-model:value="data.in_on" size="small" class="mr-4"
                                  @click="sellSubmit(1)">
                            出售中商品
                        </n-button>
            ...
            <EditGood v-if="showEditModal" :good_id="good_id" :showModal="showEditModal"
                      @checkShowModal="checkEditModal" @reloadTable="reload"></EditGood>
        </div>
    </div>
</template>
...
const columns: any = [
    ...
    },
    {
        ...
        key: "is_recommend",
        align: "center",
        render(row: { is_recommend?: any; id?: any; }) {
            return h(NSwitch, {
                size: "medium",
                color: "#1890ff",
                activeColor: "#52c41a",
...
// 已上架按钮功能实现
const sellSubmit = (sell: number) => {
    formSearch.value.is_on = sell;
    repetition();
};

// 已推荐功能实现
const recommendSubmit = (recommend: number) => {
...
//获取商品列表
const getGoodList = (params: { title?: string; is_on?: number; is_recommend?: number; current?: number; }) => {
    loadingBar.start();
...
// 上架和下架状态更改方法
const handleChangeOn = (row: { id: any; }) => {
