<template>
    <div>
        ...
        <div class="content px-4">
            ...
            <EditOrder v-if="showEditModal" :order_id="order_id" :showModal="showEditModal"
                       @checkShowModal="checkEditModal" @reloadTable="reload"></EditOrder>
        </div>
    </div>
</template>
...
const columns: any = [
    ...
    },
    {
        ...
        key: "created_at",
        align: "center",
        render(row: { status: number; id: number; }) {
            if (row.status == 2) {
                return h(NButton, {
                    size: "small",
                    color: "#1890ff",
...
// 筛选已下单商品
const orderedSubmit = (orders: number) => {
...
const getOrderList = (params: { include: string; current?: any; order_no?: string; trade_no?: string; status?: number; }) => {
    loadingBar.start();
